...
def get_word_score(word, n):
    ...
    score = max(HAND_SIZE * wordlen - 3 * (n - wordlen), 1)
...
def update_hand(hand, word):
    ...
    for letter in word:
        if letter in handCopy: 
            count = handCopy.get(letter, 0)
            if count >= 1: 
                handCopy[letter] = count - 1  
...
def is_valid_word(word, hand, word_list):
    ...
    for letter in word:
        ...
        if count == 0 and letter != "*":
            ...
            hand_copy[letter] = count - 1

    for vowel in VOWELS:
        test_word = word.replace("*", vowel)
        vowel_replaced.append(test_word)

    for item in vowel_replaced:
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while (len(hand.keys() > 1)):

        display_hand()
        word = input('Enter word, or "!!" to indicate that you are finished: ')
        
        if word == '!!':
            print("!! selected. Game over. Total points: " + score)
            break
...
def substitute_hand(hand, letter):
    ...
    for item in letters_in_hand:
        all_letters = all_letters.replace(item, '')
    all_letters = all_letters.replace(item, '')
    ...
    new_hand = hand.copy()
    del new_hand[letter]
    new_hand[random_letter] = hand[letter]
